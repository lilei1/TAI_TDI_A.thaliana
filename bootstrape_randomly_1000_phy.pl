#!/usr/bin/perl
##by Li20150716;
##randomly choose 1000 genes across ohylostrata with replacement and calculate their TAI and TDI, using the file of phylo_normalized_expression_data_NSF_nso_strand-spec_*_exp_rpkm.txt;
use strict;
use warnings;
use Statistics::R;
...
$R->set('x', $max_number);
$R->run( q 'sample_for_perl = sample(0:x,25087,replace = TRUE)' );
...
my $seedling_se_TAI = $seedling_se / $seedling_se_exp;
my $seedling_as_TAI = $seedling_as / $seedling_as_exp;

my $root_se_TAI = $root_se / $root_se_exp;
my $root_as_TAI = $root_as / $root_as_exp;

my $flower_se_TAI = $flower_se / $flower_se_exp;
my $flower_as_TAI = $flower_as / $flower_as_exp;
